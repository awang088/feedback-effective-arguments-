...
import time
from transformers import get_linear_schedule_with_warmup, ElectraTokenizer, ElectraModel
...
def get_score(y_pred, y_true):
    y_pred = torch.Tensor.cpu(torch.softmax(y_pred, dim=-1))
    y_true = torch.Tensor.cpu(y_true)
...
with timer('electra_base'):
    ...
    for fold in range(folds):
        ...
        num_warmup_steps = int(
            num_train_steps * warmup_steps_ratio
        )
            ...
            with timer(f'model_fold:{epoch}'):
                ...
                for step, (batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_target) in enumerate(valid_generator):
                    ...
                    with torch.no_grad():
                        logits = model(batch_input_ids, batch_attention_mask, batch_token_type_ids)
                        loss = criterion(logits, batch_target) 
                        _log_loss = get_score(logits, batch_target)
                    
                    val_losses.update(loss.item(), logits.size(0))
                    val_scores.update(_log_loss, logits.size(0))
                    preds = np.concatenate(
                        [preds, logits.sigmoid().to('cpu').numpy()], axis=0
                    )
                ...
                if val_losses.avg <= best_val:
                    best_val = val_losses.avg
                    best_log = val_scores.avg
                    oof[fold_array == fold] = preds
                    torch.save(model.state_dict(), MODEL_PATH_BASE +
                               f"_{fold}.pth")  # Saving current best model

final_log_loss = get_score(oof, y)
LOGGER.info(f'log score:{final_log_loss}')
