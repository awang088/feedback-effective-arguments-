...
class Collate:
    ...
    def __call__(self, batch):
        ...
        if self.isTrain:
            output["target"] = torch.tensor(np.array(output["target"]), dtype=torch.long)
...
class FeedbackModel(nn.Module):
    ...
    def __init__(self,model_name=None, config_path=None, pretrained=True, num_labels=3):
        ...
        if config_path is None:
            self.config = AutoConfig.from_pretrained(model_name, output_hidden_states=True, hidden_dropout_prob=0, attention_probs_dropout_prob=0)
        else:
    ...
    def attack_backward(self, x, y, attention_mask, epoch):
        ...
        for i in range(self.adv_step):
            ...
            with torch.cuda.amp.autocast():
                tr_logits = self.model(x, attention_mask)
...
with timer('deberta-v3-large'):
    ...
    for fold in range(folds):
        ...
        train_datagen = FeedbackDataset(train_df)
        ...
        valid_datagen = FeedbackDataset(valid_df)
